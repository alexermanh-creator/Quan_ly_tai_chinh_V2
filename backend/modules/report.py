...
class ReportModule:
    ...
    def _process_data(self):
        ...
        for h in data['holdings']:
            ...
            if sym not in symbol_stats:
                symbol_stats[sym] = {'wallet': wid, 'total_pl': 0, 'val': 0, 'cost': 0}
            symbol_stats[sym]['total_pl'] += unrealized
            symbol_stats[sym]['val'] += cur_val
            symbol_stats[sym]['cost'] += cost
        ...
        for t in transactions:
            ...
            if pl is not None:
                ...
                sym = t['symbol']
                if sym:
                    if sym not in symbol_stats: symbol_stats[sym] = {'wallet': t['wallet_id'], 'total_pl': 0, 'val': 0, 'cost': 0}
    ...
    def _generate_nav_chart_io(self, stats, raw):
        ...
        ax.plot(u_dates[-1], stats['total_assets'], marker='o', color='#d62728', markersize=8, label='Tài Sản Thực Tế (NAV)')
        ax.xaxis.set_major_formatter(mdates.DateFormatter('%m/%Y'))
        plt.title('BIỂU ĐỒ BIẾN ĐỘNG VỐN & TÀI SẢN (NAV)', fontsize=13, fontweight='bold')
        plt.ylabel('Giá trị (VNĐ)'); plt.grid(True, linestyle='--', alpha=0.6); plt.legend(loc='upper left')
    ...
    def get_telegram_report(self):
        ...
        markup.add(InlineKeyboardButton("📈 Xem Biểu Đồ Tăng Trưởng NAV", callback_data="view_nav_chart"),
    ...
    def generate_excel_bytes(self):
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            stock_v = stats['wallets']['STOCK']['assets'] + stats['wallets']['STOCK']['balance']
            crypto_v = stats['wallets']['CRYPTO']['assets'] + stats['wallets']['CRYPTO']['balance']
            matrix_d = {"Danh Mục": ["STOCK", "CRYPTO"], "Tài Sản Hiện Tại": [stock_v, crypto_v], "Lãi/Lỗ Đã Chốt": [stats['wallets']['STOCK']['realized'], stats['wallets']['CRYPTO']['realized']], "Lãi/Lỗ Đang Gồng": [stats['wallets']['STOCK']['unrealized'], stats['wallets']['CRYPTO']['unrealized']], "Tổng Lãi/Lỗ": [stats['wallets']['STOCK']['realized'] + stats['wallets']['STOCK']['unrealized'], stats['wallets']['CRYPTO']['realized'] + stats['wallets']['CRYPTO']['unrealized']]}
            pd.DataFrame(matrix_d).to_excel(writer, sheet_name="Dashboard", index=False, startrow=15)
            ...
            # 3. HISTORY SHEETS (LS Nạp Rút, STOCK, CRYPTO)
            ...
            re_d = [{"Tài Sản": "Chứng Khoán (STOCK)", "Tỷ Trọng (%)": (stock_v/tot*100) if tot else 0, "Mục Tiêu (%)": 40, "Lệch (VNĐ)": (0.4*tot) - stock_v}, {"Tài Sản": "Tiền Số (CRYPTO)", "Tỷ Trọng (%)": (crypto_v/tot*100) if tot else 0, "Mục Tiêu (%)": 40, "Lệch (VNĐ)": (0.4*tot) - crypto_v}, {"Tài Sản": "Tiền Mặt (CASH)", "Tỷ Trọng (%)": (stats['wallets']['CASH']['balance']/tot*100) if tot else 0, "Mục Tiêu (%)": 20, "Lệch (VNĐ)": (0.2*tot) - stats['wallets']['CASH']['balance']}]
            pd.DataFrame(re_d).to_excel(writer, sheet_name="Rebalancing", index=False)
        ...
        for name in wb.sheetnames:
            ws = wb[name]
            # Tự động dãn cột chuẩn (Bản sếp gửi)
            ...
            # Logic chèn báo cáo Summary vào đầu sheet lịch sử
            if name in ["LS Chứng Khoán", "LS Crypto", "LS Nạp Rút"]:
                ...
                if w_k != "CASH":
                    syms = [s for s in stats['symbols'].values() if s['wallet'] == w_k]
                    best = max(syms, key=lambda x: x['total_pl']) if syms else {'val':0}
                    worst = min(syms, key=lambda x: x['total_pl']) if syms else {'val':0}
                    sums = [f"💰 Tổng giá trị: {format_currency(w_d['assets'] + w_d['balance'])}", 
                            f"💵 Tổng vốn gốc: {format_currency(w_d['in'] - w_d['out'])}",
                            f"📈 Lãi/Lỗ: {format_currency(w_d['realized'] + w_d['unrealized'])}",
                            f"⬆️ Tổng nạp ví: {format_currency(w_d['in'])} | ⬇️ Tổng rút ví: {format_currency(w_d['out'])}"]
                else:
                    sums = [f"💰 Tổng tài sản: {format_currency(stats['total_assets'])}", 
                            f"📤 Tổng nạp: {format_currency(stats['total_in'])}", 
                            f"📥 Tổng rút: {format_currency(stats['total_out'])}",
                ...
                last = ws.max_row
                if last >= 17:
                    t_n = f"Tbl_{w_k}_{name[:3]}"
                    tab = Table(displayName=t_n[:15], ref=f"A17:{get_column_letter(ws.max_column)}{last}")
