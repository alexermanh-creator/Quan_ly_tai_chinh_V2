...
class DashboardModule:
    ...
    def get_main_dashboard(self):
        try:
            ...
            # CHỐT: Chỉ lấy Total In/Out từ Ví Mẹ
            ...
            if goal_str.startswith('lai '):
                ...
                if '%' in val: goal_target = net_invested * float(val.replace('%','')) / 100
                else:
            ...
            lines = [
                "🏦 HỆ ĐIỀU HÀNH TÀI CHÍNH V3.1", draw_line("thick"),
                ...
                f"📈 Stock: {format_currency(wallets.get('STOCK',{}).get('balance',0) + sum(h['quantity']*h['average_price'] for h in holdings if h['wallet_id']=='STOCK'))}",
                f"🟡 Crypto: {format_currency(wallets.get('CRYPTO',{}).get('balance',0) + sum(h['quantity']*h['average_price']*crypto_rate for h in holdings if h['wallet_id']=='CRYPTO'))}",
                f"🥇 Khác: {format_currency(sum(h['quantity']*h['average_price'] for h in holdings if h['wallet_id']=='OTHER'))}",
                draw_line("thick")
            ]
            ...
            for wid in ['STOCK', 'CRYPTO', 'OTHER']:
                ...
                # Tính ROI ví con dựa trên vốn đã cấp (total_in - total_out của ví con đó)
                von_ví = w['total_in'] - w['total_out']
                pl_w_pct = (w_pl[wid] / von_ví * 100) if von_ví > 0 else 0
                lines += [
