...
def show_stock(message): bot.send_message(message.chat.id, stock_mod.get_dashboard())
...
def handle_all_commands(message):
    ...
    if text.startswith(('nap ', 'rut ', 'chuyen ', 'thu ')):
        if "other" in text: # Ép logic cho ví Khác
            try:
    ...
    # 3. Cập nhật giá (up)
        ...
        try:
            ...
            rate = 1000 if price < 1000 else 1 # Tự hiểu 120 là 120k
            db.update_market_price(sym, price * rate)
            bot.reply_to(message, f"✅ {sym} = {price:,.1f}k")
    ...
    # 4. Giao dịch (s/c)
        ...
        try:
            ...
            bot.reply_to(message, f"✅ Khớp {'MUA' if qty>0 else 'BÁN'} {abs(qty)} {sym}" + (f"\n💰 Lãi: {res:,.0f}" if qty<0 else ""))
